...
def train_loop(model_name, model_path, num_epochs):
    ...
    model = EfficientNet.from_name_pruned(model_name, f'./umnist_iccv_1024/SR_{model_name}_best_512.pth', num_classes=28, device=device)
    model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if score > best_score:
            ...
            torch.save(model.state_dict(), model_path+f'pruned_SR_{model_name}_best_512.pth')
            if model_name.startswith("pruned"):
                torch.save(model, model_path+f'{model_name}_best_model_512.pt')
